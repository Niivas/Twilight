...
- **¿Para quién es esto?** **Para cualquier persona** que este buscando crear su portafolio web, desee aprender sobre desarrollo web o probar Codespaces.
- **¿Qué experiencia necesitas?** **Cero experiencia**. Tú decides cuánto deseas personalizar basado en tu experiencia y tiempo disponible.
...
6. [Personaliza el sitio de tu portafolio](#-personaliza-tu-pagina-web-en-3-pasos)
7. [Sube tu Portafolio a la nube](#-subir-tu-aplicacion-web)
...
4. Debajo del nombre del repositorio, usa el menú desplegable del Código y, en la pestaña de Codespaces selecciona "Create codespace on main".

<img src="./__images__/createcodespace.png" alt="Crea tu codespace" style="width:270px;"/>

5. Espera mientras GitHub inicializa el Codespace.
...
6. Cuando haya terminado, verás que tu Codespace comenzará con una sección de terminal en la parte inferior. Codespaces instalará todas las extensiones requeridas en tu contenedor, luego verás `npm install` ejecutando. Después de que se complete la instalación de paquetes, ve a la terminal y escribe `npm run start` para iniciar tu aplicación web en tu Codespace.
...
1. En `App.jsx`, importa tu nuevo componente `Education` en la parte superior y observa que Copilot empieza a ver que estás haciendo y dando sugerencias de autocompletado:

   ```javascript
   import Footer from "./Components/Footer";
   ```

1. Ahora agrega el componente `Education` donde deseas que se encuentre dentro de la página. Observa que Copilot sabe que deseas añadir en el componente `Education`. Este debería sugerir lo siguiente que podrás aceptar y se añadirá en tu nuevo componente:
   ```javascript
   <Education />
   ```
